...
def parse_capture(
    ...
    *,
    ...
    # DITR expects per-point pixel coordinates in (x, y) order. Since every
    # point originates from a valid RGB-D pixel, the initial visibility mask
    # is all true. ImageResize updates both coordinates and visibility.
    image_coord = np.stack((cols, rows), axis=1).astype(np.float32, copy=False)
    image_mask = np.ones(valid_points, dtype=bool)
