...
MODEL_DIR="${MODEL_DIR:-${ROOT_DIR}/models/ptv3-industrial-23cls}"
HF_MODEL_REPO="${HF_MODEL_REPO:-min99ian/ptv3-industrial-23cls}"
...
if [[ -n "${existing_id}" ]]; then
  ...
  if [[ "${owner}" != "${OWNER}" || "${service}" != "ptv3-api" ]]; then
...
docker run --detach \
  ...
  --label com.in3d.service=ptv3-api \
  ...
  --volume "${MODEL_DIR}:/models/ptv3-industrial-23cls" \
  "${hf_env[@]}" \
  --env HOME=/tmp \
  --env HOST_GPU_INDEX="${GPU_DEVICE}" \
  --env MODEL_DEVICE=cuda \
  --env HF_MODEL_REPO="${HF_MODEL_REPO}" \
  --env MODEL_DIR=/models/ptv3-industrial-23cls \
  --env MODEL_CONFIG=/models/ptv3-industrial-23cls/config.py \
  --env MODEL_WEIGHT=/models/ptv3-industrial-23cls/ptv3-industrial-23cls.pth \
